...
def build_model(
    n_filters, n_layers, seq_enc, seq_dec, 
    ...
    e_s_bias = tf.Variable(tf.random.normal(
        [1, n_stacks, seq_enc, n_filters], stddev=0.1), 
        name="e_position_stack_embeddings", trainable=True)
    e_l_bias = tf.Variable(tf.random.normal(
        [1, n_layers, seq_enc, n_filters], stddev=0.1), 
        name="e_position_layer_embeddings", trainable=True)
    d_s_bias = tf.Variable(tf.random.normal(
        [1, n_stacks, seq_dec, n_filters], stddev=0.1), 
        name="d_position_stack_embeddings", trainable=True)
    d_l_bias = tf.Variable(tf.random.normal(
        [1, n_layers, seq_dec, n_filters], stddev=0.1), 
        name="d_position_layer_embeddings", trainable=True)
    ...
    for n_stack in range(n_stacks):
        ...
        x_stack_encode = tf.add(
            x_stack_encode, e_s_bias[:, n_stack, :, :])
    ...
    for n_stack in range(n_stacks):
        ...
        x_stack_decode = tf.add(
            x_stack_decode, d_s_bias[:, n_stack, :x_length, :])
        ...
        for n_layer in range(n_layers):
            ...
            x_cnn_input = tf.add(
                x_cnn_input, d_l_bias[:, n_layer, :x_length, :])
    ...
    x_q = layers.Dense(
        n_filters, name="x_q")(x_skip_output)
    x_k = layers.Dense(
    ...
    x_out_conc = tf.concat([x_skip_output, x_att_out], axis=2)
