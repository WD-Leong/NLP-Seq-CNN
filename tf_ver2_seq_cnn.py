...
def build_model(
    n_filters, vocab_size, n_layers, n_stacks=1, kernel_size=3):
    if n_layers <= 0:
    ...
    for n_stack in range(n_stacks):
        ...
        # Skip connection. #
        x_skip_output = x_stack_input + x_residual
        x_stack_input = x_skip_output
    
    # Output logits. #
    x_logits = layers.Dense(
        vocab_size, name="logits")(x_skip_output)
...
def infer(
    model, x_input, seq_output, 
    kernel_sz=3, temp=1.0, eps=1.0e-2):
    len_input = x_input.shape[1]
    if temp <= 0:
        raise ValueError(
            "Temperature must be greater than " + str(eps) + ".")
    ...
    for n_seq in range(kernel_sz, seq_output):
        ...
        if n_seq < seq_output:
            ...
            tmp_input = tmp_inputs[:, (n_seq-len_input):n_seq]
        
        tmp_logits = model.predict(tmp_input)
        if n_seq < len_input:
            tmp_index = x_input[:, n_seq]
        else:
            tmp_probs = tf.nn.softmax(tmp_logits[:, -1, :])
            tmp_probs = tf.nn.softmax(
                tf.pow(tmp_probs, 1.0 / temp))
            tmp_index = tf.argmax(
                tmp_probs, axis=1, output_type=tf.int32)
